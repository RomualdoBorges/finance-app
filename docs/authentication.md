...
Esta etapa implementa apenas cadastro, login e observação da sessão com Firebase Authentication.

## Arquitetura

A camada visual usa hooks e não importa o Firebase. `FirebaseAuthRepository` é a única implementação da feature que chama o SDK modular (`createUserWithEmailAndPassword`, `signInWithEmailAndPassword` e `onAuthStateChanged`). `AuthService` mantém o contrato disponível para a aplicação.
...
O `AuthProvider` assina a sessão uma vez, começa em `loading`, publica `authenticated` ou `unauthenticated` e remove a assinatura ao desmontar. Ele é a única fonte da sessão; TanStack Query é usado apenas para as mutations de cadastro e login.
...
Testes unitários e de integração usam mocks do repository e não acessam rede nem Firebase real. Os E2E cobrem redirecionamento, navegação e validação básica sem submeter credenciais.
...
Continuam pendentes: login com Google, logout, recuperação e atualização de senha, verificação de e-mail, exclusão de usuário, MFA, documento do usuário, grupos e grupo ativo/padrão.
