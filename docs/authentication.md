...
Esta etapa implementa cadastro, login, logout, solicitação de recuperação de senha e observação da sessão com Firebase Authentication. O login com Google foi removido do escopo do projeto.

## Arquitetura

A camada visual usa hooks e não importa o Firebase. `FirebaseAuthRepository` é a única implementação da feature que chama o SDK modular (`createUserWithEmailAndPassword`, `signInWithEmailAndPassword`, `sendPasswordResetEmail`, `signOut` e `onAuthStateChanged`). `AuthService` mantém o contrato disponível para a aplicação.
...
O `AuthProvider` assina a sessão uma vez, começa em `loading`, publica `authenticated` ou `unauthenticated` e remove a assinatura ao desmontar. Ele é a única fonte da sessão; TanStack Query é usado apenas para as mutations de cadastro, login, logout e recuperação.
...
Continuam pendentes: atualização de senha, confirmação por código, verificação de e-mail, exclusão de usuário, MFA, documento do usuário, grupos e grupo ativo/padrão.
