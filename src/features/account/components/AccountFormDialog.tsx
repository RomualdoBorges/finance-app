...
const accountFormSchema = createAccountSchema
  .omit({ initialBalanceMinor: true })
  .extend({
    initialBalance: z
      .string()
      .refine(
        (value) => parseCurrencyToMinor(value) !== null,
        'Informe um valor em reais com no máximo duas casas decimais.',
      ),
  })

type Values = z.input<typeof accountFormSchema>
export function AccountFormDialog({
  ...
  const {
    ...
    defaultValues: {
      ...
      initialBalanceDate:
        account?.initialBalanceDate ?? format(new Date(), 'yyyy-MM-dd'),
    },
  })
  ...
  const submit = handleSubmit(async (values) => {
    try {
      const { initialBalance, ...formValues } = values
      const input = {
        ...formValues,
        initialBalanceMinor: parseCurrencyToMinor(initialBalance)!,
          ...
          >
            ...
            <Field label="Saldo inicial" error={errors.initialBalance?.message}>
              ...
              <span className="mt-1 block text-xs font-normal text-muted-foreground">
                Informe quanto já existia nesta conta na data escolhida.
              </span>
              {selectedAccountType === 'credit_card' ? (
                <span className="mt-1 block text-xs font-normal text-muted-foreground">
                  Informe dívidas existentes como valor negativo.
                </span>
              ) : null}
            </Field>
            <Field
